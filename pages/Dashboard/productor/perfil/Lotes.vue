<template>
  <div class="flex gap-8 z-0 justify-between relative flex-grow">
    ...
    >
      ...
      >
        {{ lote.length }} Lotes disponibles
      </h1>
      <div class="flex flex-wrap gap-4 justify-between mr-4">
        <div v-for="item in lote" class="w-[30%] mb-4 relative">
          <div
            class="absolute top-7 right-7 z-50 bg-white rounded-full flex text-3xl justify-center items-center transition-all ease-in-out duration-500"
          >
            <UIcon
              v-if="isEdit"
          ...
          <ProductorPerfilLotesCard :item="item" />
        </div>
      </div>
    </div>
  </div>
...
import lote_1 from "/img/lote_1.webp";
import lote_2 from "/img/lote_2.webp";
import lote_3 from "/img/lote_3.webp";
import lote_4 from "/img/lote_4.webp";
definePageMeta({
